...
*   Duy trì tư thế nằm ≥ 3 frames.
*   VÀ thoả mãn: Fast Motion ($dy_{peak} \geq 8$) HOẶC Strong Drop ($Drop \geq 0.28$) HOẶC Sustained Lie.

#### Recovery
Nếu người đứng dậy ($\theta < 55°$, $H_{curr} > 0.7 \times H_{base}$) trong 4 frames → Reset về `NORMAL`.
