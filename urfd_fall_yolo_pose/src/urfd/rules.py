...
def check_fall_candidate(features, angle_thres, ar_thres, height_drop, 
    ...
    # Check if current frame is a fall candidate via 4 paths: posture, height drop, fast motion, or high angle
...
def check_lying_posture(features, confirm_angle_thres, confirm_ar_thres):
    # Check if current frame shows lying-like posture using OR logic with relaxed thresholds
    if features["bbox"] is None:
        return False
